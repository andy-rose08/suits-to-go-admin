...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(
        `/api/${params.store_id}/categories/${data.category_id}`
      );
      router.refresh();
    ...
    <>
      ...
      <DropdownMenu>
        ...
        >
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/${params.store_id}/categories/${data.category_id}`)
            }
          >
            <Edit className="mr-2 h-4 w-4" />
            Update
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onCopy(data.category_id)}>
